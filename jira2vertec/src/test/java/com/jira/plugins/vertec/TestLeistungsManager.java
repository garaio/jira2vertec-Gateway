...
import com.jira.plugins.LeistungsManager;
...
public class TestLeistungsManager extends TestCase
{
    ...
    protected void setUp() throws Exception
    {
        ...
        bearbeiterMjr.setLoginName("foobar-user");
        ...
        leistung1.setBearbeiterId("foobar-user");
        ...
        leistung2.setBearbeiterId("foobar-user");
    ...
    public void testLeistungWirdAktualisiertBeiMutationKommentarWennKeinePhaseZugewiesen() throws VertecServiceException
    {
        ...
        Worklog worklog = CreateWorklog(bearbeiterMjr.getLoginName(), 120, defaultWorkLogId, date, defaultComment);
        leistungsManager.leistungAktualisieren(worklog, null);

        verify(repository).updateLeistung(leistung1.getId(), 120, new Date(date.getYear(), date.getMonth(), date.getDate()), config.getDefaultPhaseIdWennNichtZugeordnet(), worklogText);
    }
    ...
    private Worklog CreateWorklog(String user, int minuten, long worklogId, Date date, String comment)
    {
        ...
        when(worklog.getAuthor()).thenReturn(user);
